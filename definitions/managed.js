...
ctx =>`
    ...
    FROM 
        ${ctx.ref('CT_ext_clientes2')} cl
    JOIN 
        ${ctx.ref('CT_ext_enderecos2')} ad
        ON cl.cliente_cpf = ad.cliente_cpf
    JOIN 
        ${ctx.ref('CT_ext_itens_produtos_pedidos2')} it
        ON cl.cliente_cpf = it.pedido_cliente_cpf
    `)


transformation.transform(
    'CT_tb_itens_pedidos_clientes2', 
    'managed', 
    'table', 
    ['managed'], 
    ['CT_ext_clientes2', 'CT_ext_enderecos2', 
    'CT_ext_itens_produtos_pedidos2'], 
    query
)
