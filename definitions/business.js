...
var query = (
  ...
  SELECT
    ...
    *
  FROM 
    ${ctx.ref("CT_ext_clientes2")}
  WHERE NOT (cliente_email like "%@%.%")
  `)

assertion.assertion('raw_CT_clientes_row_condition2',['CT_ext_clientes2'],query);
...
var schema = 'business';
var tags = ['business'];
var dependencia_managed = ['CT_tb_itens_pedidos_clientes2'];
...
query = (
  ...
  SELECT DISTINCT
      ...
      cliente_estado
  FROM 
      ${ctx.ref('CT_tb_itens_pedidos_clientes2')}
  WHERE
      cliente_cpf NOT IN (SELECT cliente_cpf
                          FROM ${ctx.ref('raw_CT_clientes_row_condition2')})  
  `);

transformation.transform(
  nome_da_tabela = 'CT_marketing_clientes2', 
  schema, 
  operacao = 'view', 
  tags, 
  dependencias = ['CT_tb_itens_pedidos_clientes2', 
        'raw_CT_clientes_row_condition2'], 
  query
);
...
transformation.transform(
  nome_da_tabela = 'CT_adm_receita_MG2', 
  schema, 
  operacao='view', 
  tags, 
  dependencia_managed, 
...
transformation.transform(
  nome_da_tabela = 'CT_adm_receita_SP2', 
  schema, 
  operacao='view', 
  tags, 
  dependencia_managed, 
...
transformation.transform(
  nome_da_tabela = 'CT_adm_receita_por_dia2', 
  schema, 
  operacao='view', 
  tags, 
  dependencia_managed, 
...
transformation.transform(
  nome_da_tabela = 'CT_adm_receita_por_estado2', 
  schema, 
  operacao='view', 
  tags, 
  dependencia_managed, 
